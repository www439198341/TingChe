			...
			try {
				if (!rs1.next()) {// 用户不存在，注册。此时返回必须绑定车牌号的信息。
					...
					if (rs2.next()) {// 获得默认车牌号
						...
						// System.out.println("rs3 is open");
						if (rs3.next()) {// 在停车场，有未完成的停车信息
							// 关闭rs3的资源
							SqlHelper.close(rs3, SqlHelper.getPs(), SqlHelper.getCt());
							// 查询是否已经首次支付
							sql = "select firstpay from parkinfo where iscompleted=0 and carnumber=(select carnumber1 from userinfo where openid = ?)";
							parameters = new String[] { openid };
							ResultSet rs4 = SqlHelper.executeQuery(sql, parameters);
							// System.out.println("rs4 is open");
